...
from qa.checks.models import Inputs, Metadata
...
class TestValidTitleCheck:
    ...
    def test_fail_empty_has_sub_results(self):
        ...
        not_empty = next(r for r in result.results if r.check_name == "not_empty")
        assert not not_empty.passed

    def test_warn_too_short(self):
        result = ValidTitleCheck.check("Tiny")
        assert result.passed
        too_short = next(r for r in result.results if r.check_name == "not_too_short")
        assert not too_short.passed

    def test_warn_lowercase_start(self):
        result = ValidTitleCheck.check("a title with lowercase")
        assert result.passed
        lowercase = next(r for r in result.results if r.check_name == "does_not_start_with_lowercase")
        assert not lowercase.passed
    ...
    def test_warn_leading_whitespace(self):
        ...
        assert result.passed
        check = next(r for r in result.results if r.check_name == "no_boundary_whitespace")
    ...
    def test_warn_multiple_spaces(self):
        ...
        assert result.passed
        check = next(r for r in result.results if r.check_name == "no_extra_whitespace")
        assert not check.passed
    ...
    def test_warn_excessive_caps(self):
        ...
        assert result.passed
        check = next(r for r in result.results if r.check_name == "no_excessive_capitals")
        assert not check.passed
    ...
    def test_pass_one_long_caps_word(self):
        ...
        assert result.passed
        check = next(r for r in result.results if r.check_name == "no_unapproved_long_caps_words")
        assert check.passed
    ...
    def test_warn_html_sup(self):
        ...
        assert result.passed
        check = next(r for r in result.results if r.check_name == "no_html_elements")
        assert not check.passed
    ...
    def test_warn_begins_with_title(self):
        ...
        assert result.passed
        check = next(r for r in result.results if r.check_name == "does_not_begin_with_title")
        assert not check.passed
    ...
    def test_warn_linebreak(self):
        ...
        assert result.passed
        check = next(r for r in result.results if r.check_name == "does_not_contain_linebreak")
        assert not check.passed
