...
from qa.checks.submission.files import DoesNotExceedTheFileSizeLimit, AcceptableFileType
...
class TestAcceptableFileType:
    def test_filetype_pass(self):
        sub_metadata = make_test_submit_event_info()
        result = AcceptableFileType().run(QaDataRegistry(submit_event_info=sub_metadata))

        assert result.passed

    def test_filetype_fail(self):
        sub_metadata = make_test_submit_event_info(source_format="html")
        result = AcceptableFileType().run(QaDataRegistry(submit_event_info=sub_metadata))
