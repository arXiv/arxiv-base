...
* **"now" is always injected.** None of the functions under test are allowed
  ...
  Every test passes an explicit ``datetime``.
...
This is the pure half: every function here takes the holiday calendar as a
``frozenset`` and touches no database. The DB-backed loader and the localtime
aggregate are covered in ``test_localtime_db.py``.

The holiday calendar comes from :data:`tests.conftest.HOLIDAY_DATES`:
...
# ==========================================================================
# The module must not drag in a database
# ==========================================================================

class TestNoDatabaseImport:
    """The reason this module is split from ``localtime_db``.

    ``arxiv.db``'s package load calls ``configure_db()``, which builds an
    engine from ``CLASSIC_DB_URI``. If that ran merely because something
    asked whether today is a publish day, every caller would need a
    configured database -- and a caller that connects by other means (the
    Cloud SQL Python Connector, with no URI to give) fails outright at
    import.
    """

    def test_importing_does_not_pull_in_arxiv_db(self) -> None:
        """Checked in a subprocess: this pytest session has already imported
        arxiv.db via other test modules, so an in-process assertion would
        pass vacuously no matter what this module does.
        """
        code = ("import sys; import arxiv.submission.arxiv_localtime; "
                "print('arxiv.db' in sys.modules)")
        out = subprocess.run([sys.executable, "-c", code],
                             capture_output=True, text=True, check=True)
        assert out.stdout.strip() == "False", (
            "importing arxiv_localtime pulled in arxiv.db -- the DB-free "
            "guarantee is broken; something added a module-scope import"
        )
