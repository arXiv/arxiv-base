...
from .exceptions import AbsException, AbsParsingException, AbsNotFoundException


RE_ABS_COMPONENTS = re.compile(r"^\\\\\n", re.MULTILINE)
RE_FROM_FIELD = re.compile(r"(?P<from>From:\s*)(?P<name>[^<]+)?\s+(<(?P<email>.*)>)?")
...
RE_ARXIV_ID_FROM_PREHISTORY = re.compile(r"(Paper:\s+|arXiv:)(?P<arxiv_id>\S+)")
...
def parse_abs(raw: str, modified: datetime) -> DocMetadata:
    ...
    missing = [
        rf for rf in REQUIRED_FIELDS if not hasattr(abs, rf) or not getattr(abs, rf)
    ]
    if missing:
...
def parse_abs_top(raw: str, modified: datetime, abstract: str) -> DocMetadata:
    ...
    fields: Dict[str, Any] = _parse_metadata_fields(key_value_block=misc_fields)
    ...
    if parsed_version_entries:
        (version, version_history, arxiv_id_v) = _parse_version_entries(
...
def _parse_version_entries(
    arxiv_id: str, version_entry_list: List
    ...
    for parsed_version_entry in version_entry_list:
        version_count += 1
        date_match = RE_DATE_COMPONENTS.match(
            parsed_version_entry
        ) or RE_REP_COMPONENTS.match(parsed_version_entry)
        if not date_match:
            raise AbsParsingException(
    ...
    return (version_count, version_entries, f"{arxiv_id}v{version_entries[-1].version}")
...
def alt_component_split(components: List[str]) -> List[str]:
    ...
    """
    if len(components) <= 4:
        raise AbsParsingException("Unexpected number of components parsed from .abs.")
    alt_comp = []
    abstract = ""
