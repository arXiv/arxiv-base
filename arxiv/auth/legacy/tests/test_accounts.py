...
def get_user(session, user_id):
    ...
    db_profile = (
        session.query(models.Demographic)
...
def test_register_with_name_details(app):
    ...
    user = domain.User(username="bazuser", email="new@account.edu", name=name)
    ip = "1.2.3.4"
    user_id = None
    with app.app_context():
...
def test_register_with_profile(app):
    ...
    name = domain.UserFullName(forename="foo", surname="user", suffix="iv")
    user = domain.User(
        username="bazuser", email="new@account.edu", name=name, profile=profile
    )
    ip = "1.2.3.4"
    ...
    with app.app_context():
        with transaction() as session:
            ...
            auth_user, auths = authenticate.authenticate(
                username_or_email=user.username, password="apassword1"
            )
...
def test_update_name(app):
    ...
    with app.app_context():
        with transaction() as session:
            updated_name = domain.UserFullName(
                forename="Foo", surname=name.surname, suffix=name.suffix
            )
            updated_user = domain.User(
