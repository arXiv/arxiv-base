        ...
        with self.app.app_context():
            ...
            with transaction() as session:
                for pattern in ok_patterns:
                    session.execute(
                        insert(models.t_arXiv_white_email).values(pattern=str(pattern))
                    )
                for pattern in bad_patterns:
                    session.execute(
                        insert(models.t_arXiv_black_email).values(pattern=str(pattern))
                    )
...
class TestAutoEndorsement(TestCase):
    ...
    def test_domain_papers(self):
        ...
        with self.app.app_context():
            with transaction() as session:
                ...
                session.execute(
                    insert(models.t_arXiv_in_category).values(
                        document_id=1, archive="cs", subject_class="DL", is_primary=1
                    )
                )
                ...
                session.execute(
                    insert(models.t_arXiv_in_category).values(
                        document_id=2, archive="cs", subject_class="IR", is_primary=1
                    )
                )
                # session.add(models.Category(
                #     archive='cs',
    ...
    def test_is_academic(self):
        ...
        with self.app.app_context():
            ...
            self.assertTrue(
                endorsements.is_academic(
                    domain.User(
                        user_id="2", email="someone@fsu.edu", username="someone"
                    )
                )
            )
            self.assertFalse(
                endorsements.is_academic(
                    domain.User(
                        user_id="2", email="someone@fsu.biz.edu", username="someone"
                    )
                )
            )
            self.assertTrue(
                endorsements.is_academic(
                    domain.User(
                        user_id="2", email="someone@aaas.org", username="someone"
                    )
                )
            )
            self.assertFalse(
                endorsements.is_academic(
                    domain.User(
                        user_id="2", email="someone@foo.com", username="someone"
                    )
                )
            )
