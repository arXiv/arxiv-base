...
class SetUpUserMixin(TestCase):
    ...
    def setUp(self):
        ...
        settings = Settings(CLASSIC_DB_URI=self.db_uri, LATEXML_DB_URI=None)
        ...
        with self.app.app_context():
            ...
            self.user_class = Session.scalar(
                select(models.TapirPolicyClass).where(
                    models.TapirPolicyClass.class_id == 2
                )
            )
            ...
            self.db_password = models.TapirUsersPassword(
                user_id=self.user_id, password_storage=2, password_enc=hashed
            )
