...
from ...db.models import TapirUser, TapirUsersPassword, TapirNickname, Demographic
...
def does_username_exist(username: str) -> bool:
    ...
    """
    data = (
        Session.query(TapirNickname).filter(TapirNickname.nickname == username).first()
    )
    if data:
        return True
    return False
...
def register(
    user: domain.User, password: str, ip: str, remote_host: str
    ...
    if user.profile is not None:
        if db_profile is not None:
            ...
            _update_field_if_changed(
                db_profile, "affiliation", user.profile.affiliation
            )
            _update_field_if_changed(db_profile, "country", user.profile.country)
            _update_field_if_changed(db_profile, "type", user.profile.rank)
            _update_field_if_changed(db_profile, "url", user.profile.homepage_url)
            _update_field_if_changed(
                db_profile, "archive", user.profile.default_archive
            )
            _update_field_if_changed(
                db_profile, "subject_class", user.profile.default_subject
            )
            for grp, field in Demographic.GROUP_FLAGS:
                _update_field_if_changed(db_profile, field, _has_group(grp))
...
def _get_user_data(user_id: str) -> Tuple[TapirUser, TapirNickname, Demographic]:
    try:
        db_user, db_nick = (
            Session.query(TapirUser, TapirNickname)
    ...
    db_profile = (
        Session.query(Demographic).filter(Demographic.user_id == user_id).first()
    )
    if not db_user:
        raise exceptions.NoSuchUser("User does not exist")
...
def _create(
    ...
    db_nick = TapirNickname(
        user=db_user, nickname=user.username, flag_valid=1, flag_primary=1
    )
    Session.add(db_nick)
    ...
    db_pass = TapirUsersPassword(
        user=db_user, password_storage=2, password_enc=hash_password(password)
    )
    Session.add(db_pass)
