    ...
    def test_relative_static_urls(self):
        ...
        with self.app.app_context():
            ...
            self.assertTrue(
                ...
                "The static URL for base starts with the configured prefix.",
            )
            url = url_for("fooprint.static", filename="img/foo.jpg")
            self.assertFalse(
                url.startswith("http://nope.com/oof"),
...
class TestGetURLMap(TestCase):
    ...
    def test_no_urls_configured(self):
        ...
        self.assertEqual(len(adapter.map._rules), 0, "No URL patterns are loaded")

    @mock.patch(
        "arxiv.base.urls.base_config",
        mock.MagicMock(
            URLS=[
                ("foo", "/foo", "foo.org"),
    ...
    def test_base_urls(self):
        ...
        app = mock.MagicMock(config={"URLS": []})
        adapter = urls.build_adapter(app)
        self.assertEqual(len(adapter.map._rules), 2, "Only base URLs are loaded")
    ...
    def test_overlapping_urls(self):
        ...
        adapter = urls.build_adapter(app)
        self.assertEqual(len(adapter.map._rules), 3, "Duplicate URLs not added")
        self.assertEqual(
            adapter.build("baz"),
            "https://baz.org/baz",
...
class TestExternalURLFallback(TestCase):
    ...
    def test_url_from_app_config(self):
        ...
        Base(self.app)
        with self.app.app_context():
            self.assertEqual(url_for("bat", foo="yes"), "https://bat.org/bat/yes")
...
class TestWithClient(TestCase):
    ...
    def test_application_url(self):
        """url_for works as expected for an app-defined URL."""
        self.assertEqual(self.client.get("/baz_location").data, b"https://baz.org/baz")
        self.assertEqual(
            self.client.get("/bat_location").data, b"https://bat.org/bat/1"
        )
        self.assertEqual(
            self.client.get("/acknowledgment_location").data,
