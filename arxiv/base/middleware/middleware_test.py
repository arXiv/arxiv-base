...
class TestWrap(TestCase):
    ...
    def test_wrap_attaches_middleware(self):
        ...
        self.assertIsInstance(
            app.wsgi_app, FooMiddleware, "Middleware should be attached to Flask app"
        )
        app(self.environ, mock.MagicMock())
        self.assertTrue(
            app.wsgi_app.called, "Middleware should be called when Flask app is called"
        )
    ...
    def test_wrap_is_inside_out(self):
        ...
        app(self.environ, mock.MagicMock())
        self.assertEqual(
            self.environ["call_order"][0],
            "first",
            "The first middleware should be called first",
        )
        self.assertEqual(
            self.environ["call_order"][1],
            "second",
...
class TestRequestLogs(TestCase):
    ...
    def test_uwsgi_is_available(self):
        ...
        self.assertIsInstance(
            app.wsgi_app,
            request_logs.ClassicLogsMiddleware,
            "ClassicLogsMiddleware should be the outermost middleware",
        )
        app(self.environ, mock.MagicMock())
        self.assertGreater(
            mock_uwsgi.set_logvar.call_count,
            0,
