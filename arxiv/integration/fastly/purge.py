...
def _purge_category_change(
    arxiv_id: Identifier, old_cats: Optional[str] = None
) -> List[str]:
    ...
    if recent_date:
        if (
            today - timedelta(days=3) <= recent_date
        ):  # farthest away a date on the new page would likely be
            new = True
    ...
    if old_cats:  # clear any pages this paper may have been removed from or added to
        old_groups, old_archives, old_categories = get_all_cats_from_string(
            old_cats, True
        )
...
def purge_fastly_keys(
    key: Union[str, List[str]],
    service_name: Optional[str] = "arxiv.org",
    soft_purge: Optional[bool] = False,
):
    ...
    with fastly.ApiClient(configuration) as api_client:
        ...
        if isinstance(key, str):
            try:
                api_response = _purge_single_key(
                    key, SERVICE_IDS[service_name], api_instance, soft_purge
                )
                logger.info(
                    f"Fastly Purge service: {service_name}, key: {key}, status: {api_response.get('status')}, id: {api_response.get('id')}"
                )
            except fastly.ApiException as e:
                logger.error(
                    f"Exception purging fastly key(s): {e} service: {service_name}, key: {key}"
                )
                raise
        else:
            try:
                _purge_multiple_keys(
                    key, SERVICE_IDS[service_name], api_instance, soft_purge
                )
                logger.info(
                    f"Fastly bulk purge complete service: {service_name}, keys ({len(key)}): {key}"
                )
            except fastly.ApiException as e:
                logger.error(
                    f"Exception purging fastly key(s): {e} service: {service_name}, for {len(key)} keys"
                )
                raise


def _purge_single_key(
    key: str, service_id: str, api_instance: PurgeApi, soft_purge: bool = False
) -> Any:
    """purge all pages with a specific key from fastly, fastly will not indicate if the key does not exist"""
    options = {"service_id": service_id, "surrogate_key": key}
    if soft_purge:
        options["fastly_soft_purge"] = 1
    return api_instance.purge_tag(**options)


def _purge_multiple_keys(
    keys: List[str], service_id: str, api_instance: PurgeApi, soft_purge: bool
):
    ...
    if len(keys) > MAX_PURGE_KEYS:
        _purge_multiple_keys(
            keys[0:MAX_PURGE_KEYS], service_id, api_instance, soft_purge
        )
        _purge_multiple_keys(
            keys[MAX_PURGE_KEYS:], service_id, api_instance, soft_purge
        )
    else:
        options = {
