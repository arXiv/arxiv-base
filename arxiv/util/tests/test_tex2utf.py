...
class TextTex2Utf(TestCase):
    def test_tex2utf(self):
        ...
        test_str = "\\ddag \\OE\\S"
        utf_out = tex2utf(test_str)
        self.assertEqual(utf_out, chr(0x2021) + chr(0x0152) + chr(0x00A7))
        ...
        utf_out = tex2utf(test_str)
        self.assertEqual(utf_out, chr(0x03B1) + chr(0x03B2) + chr(0x03B3))
        ...
        utf_out = tex2utf(test_str)
        self.assertEqual(utf_out, chr(0x03B1) + " " + chr(0x03B2) + "{something}")

        # use "\ " as textlet delimiter
        test_str = "foo \\OE\\ bar"
        utf_out = tex2utf(test_str)
        self.assertEqual(utf_out, "foo " + chr(0x0152) + " bar")
        ...
        utf_out = tex2utf(test_str)
        self.assertEqual(utf_out, "foo " + chr(0x03B1) + "_7")
    ...
    def test_tex2utf_underscore(self):
        ...
        utf_out = tex2utf(test_str)
        self.assertEqual(
            utf_out, "foo " + chr(0x03B1) + "_" + chr(0x03B2) + "_" + chr(0x03B3) + "_7"
        )
    ...
    def test_ARXIVOPS805(self):
        """{\aa} wasn't being converted correctly in /abs abstract field to
        å."""
        self.assertEqual(tex2utf("ARXIVOPS-805 \\aa  fix"), "ARXIVOPS-805 å fix")

        self.assertEqual(
            tex2utf("ARXIVOPS-805 \\aa  fix", greek=False), "ARXIVOPS-805 å fix"
        )

        self.assertEqual(
            tex2utf("ARXIVOPS-805 \\aa  fix", greek=True), "ARXIVOPS-805 å fix"
        )
