...
class Group(BaseTaxonomy):
    ...
    def get_archives(self, include_inactive: bool = False) -> List["Archive"]:
        ...
        if include_inactive:
            return [
                archive for archive in ARCHIVES.values() if archive.in_group == self.id
            ]
        else:
            return [
                archive
                for archive in ARCHIVES.values()
                if archive.in_group == self.id and archive.is_active
            ]
    ...
    def get_categories(self, include_inactive: bool = False) -> List["Category"]:
        ...
        if include_inactive:
            return [
                ...
                if category.in_archive == self.id
            ]
        else:
            return [
                category
                for category in CATEGORIES.values()
                if category.in_archive == self.id and category.is_active
            ]
...
def get_all_cats_from_string(
    cat_string: str, only_cannonical: Optional[bool] = False
) -> Tuple[List[Group], List[Archive], List[Category]]:
