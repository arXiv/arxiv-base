...
warnings.warn(
    "Deprecated: Do not use. each package should use WTForms CSRF as needed",
    DeprecationWarning,
)
...
class CSRFForm(Form):
    ...
    class Meta:
        ...
        def csrf_context(self) -> Dict[str, str]:
            ...
            if not request or (
                not hasattr(request, "session") and not hasattr(request, "auth")
            ):
            ...
            return {"ip_address": getattr(request, "remote_addr"), "nonce": nonce}
