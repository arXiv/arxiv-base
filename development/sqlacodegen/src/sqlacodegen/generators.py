...
class TablesGenerator(CodeGenerator):
    ...
    def get_class_name_override(self, table_name: str, suggested: str):
        opt = self.model_metadata.get(table_name)
    ...
    def generate_model_name(self, model: Model, global_names: set[str]) -> None:
        preferred_name = self.get_class_name_override(model.table.name, f"t_{model.table.name}")
...
class DeclarativeGenerator(TablesGenerator):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(metadata, bind, options, indentation=indentation, model_metadata=model_metadata)
    ...
    def generate_model_name(self, model: Model, global_names: set[str]) -> None:
        if isinstance(model, ModelClass):
            ...
            if model.table.name in self.model_metadata:
                preferred_name = self.get_class_name_override(model.table.name, preferred_name)
            else:
                preferred_name = "".join(
    ...
    def render_class(self, model: ModelClass) -> str:
        ...
        rendered_relationship_attributes: list[str] = [
            self.render_relationship(relationship, model.table)
            for relationship in model.relationships
        ]
    ...
    def render_relationship(self, relationship: RelationshipAttribute, table: Table) -> str:
...
class SQLModelGenerator(DeclarativeGenerator):
    ...
    def render_relationship(self, relationship: RelationshipAttribute, table: Table) -> str:
        rendered = super().render_relationship(relationship, table).partition(" = ")[2]
